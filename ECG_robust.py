...
def _apply_hamming_for_1_chunk(binary_chunk: str) -> str:
    ...
    chunk_length = 4
    p1 = (chunk_length - (int(binary_chunk[0])+ int(binary_chunk[1])+ int(binary_chunk[3])) )%2
    p2 = (chunk_length - (int(binary_chunk[0])+ int(binary_chunk[2])+ int(binary_chunk[3])) )%2
    p3 = (chunk_length - (int(binary_chunk[1])+ int(binary_chunk[2])+ int(binary_chunk[3])) )%2
...
binary_ssn       = convert_ssn_to_binary(user_ssn)
...
# =======================
subsequence_length = 3* len(ssn_with_hamming) # m = 3*l (from paper)
num_subsequences   = 4 # from me
n_timesteps        = math.floor(subsequence_length * num_subsequences) # from paper
...
power = 0.01 # power, used 0.01-0.1 range (paper doesnt give value)
for ecg_subseq in ecg_subsequences:
    subseq_fft, magnitudes, phase_angles = get_fourier_terms(ecg_subseq)
 
    modified_magnitude = apply_watermark_to_subsequences(watermark, magnitudes, power)
    ...
    print(get_mae(ecg_subseq, watermarked_subseq))
...
freqs      = np.fft.fftfreq(n_timesteps, d=1/fs)
